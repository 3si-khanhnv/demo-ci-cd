...
export function guessLocaleFormat() {
  const languages = navigator.languages || (navigator.language ? [navigator.language] : false);
  const loadLocale = (locale: string) => {
    if (!localeFormatSettings.includes(locale)) return;
    dayjs.locale(locale);
    return true;
  };
  ...
  if (languages) {
    languages.some((lang) => {
      ...
      return loadLocale(lang) || loadLocale(lang.split("-")[0]);
    });
  }
}
